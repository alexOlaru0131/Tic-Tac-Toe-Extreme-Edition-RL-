...
class Agent_SARSA:
        def __init__(
                ...
                self.env = env
                self.n_states = 3 ** env.observation_space.shape[0]
        ...
        def _to_key(self, obs: np.ndarray) -> tuple:
                base3 = ((np.array(obs) + 1).astype(int)).tolist()  # [-1,0,1] -> [0,1,2]
                index = 0
                for i, val in enumerate(base3):
                        index += val * (3 ** i)
                return index
        ...
        def update(
                ...
                self.q_values[observation, action] += self.learning_rate * \
                        (
